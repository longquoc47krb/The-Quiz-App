...
export const fetchResultsByQuizId = async (id: string) => {
  try {
    const response = await axiosClient.get<Result>(`/result/quiz/${id}`);
    return response.data;
  } catch (error) {
    // Handle the error here
...
export const fetchResultsByPlayerIdAndQuizId = async (id: string, quizId: string) => {
  try {
    const response = await axiosClient.get<Result>(`/result/quiz/user/${id}`, { params: {
      quizId
    }});
    return response.data;
  } catch (error) {
