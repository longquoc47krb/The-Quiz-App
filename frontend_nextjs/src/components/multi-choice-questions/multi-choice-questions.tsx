...
import { checkOptionIsCorrectOrNot, replaceHtmlComma } from "@/utils";
...
const MultiChoiceQuestions = ({
  ...
  return (
    <div className="min-w-fit relative">
      ...
      <div
        className={`md:grid ${
          question.options.length === 3 ? "md:grid-cols-1" : "md:grid-cols-2"
        } md:gap-4 mt-4 flex flex-col gap-y-4 relative`}
      >
        ...
        {showScore && (
          <motion.h1
            ...
            className={`${
              handleScore(timePerQuestion, isCorrect) > 0
                ? "text-lime-500"
                : "text-red-500"
            } px-4 py-2 rounded-2xl text-right font-bold w-full text-2xl absolute -top-10 right-0 glitch`}
          >
            +{scorePerQuestion}
          </motion.h1>
        )}
        {isShowExplain && streak % 5 === 0 && (
          <motion.h1
            ...
            animate={{
              opacity: 1,
              scale: [1.05, 1, 1.05, 1, 1.05, 1, 1.05, 1, 1, 1.05, 1, 1.05, 1],
            }}
      ...
      {isShowExplain && (
        <div className="bg-gray-600 text-gray-900 rounded-lg p-4 mt-4 w-full fade-in flex justify-center"  dangerouslySetInnerHTML={{__html: question.explain}} />
      )}
    </div>
  );
};
